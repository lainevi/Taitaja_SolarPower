...
class AuroraBaseClient(object):
    ...
    def check_crc(self, response):
        ...
        if response[6:8] != self.crc(response[0:6]):
            ...
            print('Response has a wrong CRC')
            return 0
        else:
            return 1
    ...
    def check_transmission_state(self, response):
        ...
        if ts == 0:
            ...
            print(Mapping.TRANSMISSION_STATES[ts])
            return 0
        else:
            #raise AuroraError('Unknown transmission state')
            print('Unknown transmission state')
            return 0
    ...
    def state(self, state_type, mapped=True):
        ...
        if not (self.check_crc(response) and self.check_transmission_state(response)):
            return ['N/A','N/A','N/A','N/A','N/A']
...
class AuroraSerialClient(AuroraBaseClient):
    ...
    def connect(self):
        #print("Opening RS485 port", self.serline.name)
        try:
            self.serline.open()
        except SerialException as e:
            #print("Failed to Connect", self.serline.name)
            raise AuroraError(str(e))
        #else:
            #print("RS485 port opened")
    ...
    def send_and_recv(self, request):
        ...
        # Enable RS-485 Tx, RS-485 works through GPIO pins 14, 15 and transmission enable bit is GPIO18
        # Set GPIO18(Board pin 12) mode to out and enable transmission
        GPIO.setmode(GPIO.BOARD)
        GPIO.setup(12,GPIO.OUT)
        GPIO.output(12,True)

        # Write request to serial line
        #print("Writing to", self.serline.name + ":", request)
        self.serline.write(request)
        
        # Wait that sending is completed
        while self.serline.out_waiting:
            print("Still sending...")
            time.sleep(0.001)
        # For some reason needs time still to finish sending now 5ms, 1ms wont work anymore
        time.sleep(0.005)
        self.serline.flushOutput()
        #self.serline.flushInput()
        # Disable transmission GPIO18(pin12)
        ...
        i=1        
        while 1:
            inbuffer = self.serline.in_waiting
            
            #print("Try"+str(i)+": in_waiting,", self.serline.in_waiting)
            if inbuffer>=8:
                # Clear preceeding ghost byte if length is 10 or more
                if inbuffer > 9:
                    self.serline.read(inbuffer-9)
                
                # Read only 8 bytes because there is sometimes trailing ghost byte
                self.serline.readinto(response)
                #print("Bytes read: ", self.serline.readinto(response), response)
                # Then flush rest
                self.serline.flushInput()
                return response
            else:
                #print("Wait for proper answer!!")
                time.sleep(0.05)
            
            if time.time()-start_time > self.timeout:
                print("Failed to receive answer!")
                return bytearray([255,0,0,0,0,0,0,0])
